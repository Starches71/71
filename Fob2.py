...
url_txt_file = os.path.join(auto_dir, "url.txt")  # Path to save the URL
...
if response.status_code == 200:
    ...
    if "items" in data and len(data["items"]) > 0:
        ...
        # Save the image URL to the new 'url.txt' file
        with open(url_txt_file, "w") as url_file:
            url_file.write(image_url + "\n")

        print(f"Image URL saved in {url_txt_file}")
