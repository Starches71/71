...
def download_audio_only(link, out_dir):
    try:
        ...
        subprocess.run([
            ...
            "--proxy", "socks5://127.0.0.1:9050",
            "-f", "bestaudio",
            "-o", os.path.join(out_dir, "Aud.%(ext)s"),
            link
        ], check=True)
    except subprocess.CalledProcessError as e:
...
def download_thumbnail(link, out_dir):
    try:
        ...
        subprocess.run([
            ...
            "-o", os.path.join(out_dir, "thumb"),
            link
        ], check=True)

        # Rename the thumbnail
        for fname in os.listdir(out_dir):
            if fname.endswith(".jpg") and fname.startswith("thumb"):
                os.rename(os.path.join(out_dir, fname), os.path.join(out_dir, "thumb.jpg"))
                break
    except subprocess.CalledProcessError as e:
