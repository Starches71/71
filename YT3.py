...
TRANSCRIPT_DIR = "Vid/Transcript"   # Ensure GEMINI.py saves here
...
def save_binary_file(file_path, data):
    ...
    with open(file_path, "wb") as f:
        f.write(data)
    print(f"Saved TTS audio: {file_path}")
...
def main():
    ...
    if not os.path.isdir(TRANSCRIPT_DIR):
        raise NotADirectoryError(f"{TRANSCRIPT_DIR} is not a directory.")
    ...
    for filename in txt_files:
        ...
        if text:
            ...
            print(f"Transcript empty after cleaning: {filename}")
