...
import subprocess                       
# Directory paths
input_dir = "best_vid"                  output_dir = "best_vid_clean"
                                        # Create output directory if it doesn't exist
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

# Process each file in the input directory                                      for video_file in os.listdir(input_dir):    if ".mp4" in video_file:  # Process files containing .mp4 in their names
        ...
        for idx in range(5):
            ...
            base_name = video_file.rsplit('.', 2)[0]  # Remove the two extensions (e.g., .mp4.mkv -> base name)
