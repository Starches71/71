...
import os
import subprocess                                                               # Paths for directories
descriptions_dir = "best_descriptions"  cleaned_dir = "best_clean"

# Static intro and outro templates      intro_templates = [                         "Coming in at number 7, we have {Hotel Name}",
    "At number 6 we have {Hotel Name},",    "Number 5 on the list is {Hotel Name},",                                        "Number 4 is {Hotel Name},",
    "Taking the number 3 spot is {Hotel Name},",
    "Coming in at number 2 is {Hotel Name},",                                       "And finally, number 1 today is {Hotel Name}"
]
...
for index, hotel_file in enumerate(hotel_files):
    ...
    # Get the corresponding intro and outro for this hotel (adjusted by index to match intro templates correctly)
    intro = intro_templates[6 - index].replace("{Hotel Name}", hotel_file.split(".")[1])  # Reverse the order for correct matching
    outro = outro_templates[6 - index].replace("{Hotel Name}", hotel_file.split(".")[1])  # Reverse the order for correct matching
    ...
    cleaned_file_path = os.path.join(cleaned_dir, f"{hotel_number}. {hotel_file.split('.')[1]}.clean")
